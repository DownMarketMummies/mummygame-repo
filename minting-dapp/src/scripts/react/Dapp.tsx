        ...
        :
          <div className="no-wallet">
            ...
            <div className="use-block-explorer">
             <strong> Mummy Game </strong>is a battle between thousands of mummies competing for $TREATZ as they fight to save Ethypt. <span className="emoji">🧟</span>The first Halloween 2022 play-to-earn.<span className="emoji">🎃</span> <br />Mummies rise together, and as a community we will <strong>ALWAYS</strong> be prepared to band together in down times. <span className="emoji">🧟</span> Huge Halloween airdrop for holders on OCT 31.<br />
             <br />
              Happy minting and keep safe! <span className="emoji">🧡</span>
            </div>
            ...
            {!this.isWalletConnected() || this.state.isWhitelistMintEnabled ?
              <div className="merkle-proof-manual-address">
                ...
                <label htmlFor="merkle-proof-manual-address"></label>
                <input id="merkle-proof-manual-address" type="text" placeholder="0x000..." disabled={this.state.userAddress !== null} value={this.state.userAddress ?? this.state.merkleProofManualAddress} ref={(input) => this.merkleProofManualAddressInput = input!} onChange={() => {this.setState({merkleProofManualAddress: this.merkleProofManualAddressInput.value})}} /> <button onClick={() => this.copyMerkleProofToClipboard()}>Click the connect wallet button above to mint.</button>
              </div>
              : null}
          </div>
        }
